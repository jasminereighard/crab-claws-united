...
#run linear regression of MO2 ~ po2 when crabs are oxyconforming
#Index MO2 data for final 4 MO2 measurement

pcrit_lm1 <- lm(MO2 ~ avg.po2, data = pcrit_MO2_1[28:32, ])
pcrit_lm1


#residual standard error is the e in our pcrit
#line should intersect with 0 
#these crabs may have some other regulatory mechanisms (C1 R1)

pcrit_model1 <- summary(pcrit_lm1)

#use pcrit function to find that shit

findPcrit(pcrit_model1, SMR = 200)
...
```{r}
pcrit_MO2_1 <- read.table(file = "/Users/jasminereighard/Desktop/mbio621/raw_data/C2_rawdata/Experiment_PCRIT_TC__C2 22 November 2024 01 06PM/Summary data resp 1.txt", 
                        skip = 15,
                        header = TRUE,
                        fill = TRUE,
                        sep = ";")

pcrit_MO2_1$Clock.TIME <- as.POSIXct(pcrit_MO2_1$Clock.TIME)

#quick visualization of MO2 vs time / po2
plot(pcrit_MO2_1$MO2 ~ pcrit_MO2_1$Clock.TIME)
plot(pcrit_MO2_1$MO2 ~ pcrit_MO2_1$avg.po2)
...
```{r}
# Define temperature, mass, and group vectors
temperature <- c(26, 26, 26, 26)
mass <- c(31.95, 25.61, 33.6, 25.46)
group <- c(rep("Control", 4))

# Create the wide format dataframe
# Each observation is represented in a single row
widerespdat <- data.frame(
  id = names(control_q20),
  temperature = temperature,
  mass = mass,
  group = group,
  MMR = unlist(control_MMR),
  SMR = unlist(control_q20),
  AS = unlist(aerobic_scope_values)
)


#change to long format
#each unique measurement or observation is represented as a separate row, and additional columns are used to indicate the variable or condition (i.e., metric = SMR, MMR, AS).
longrespdat <- pivot_longer(widerespdat, cols = c(SMR, MMR, AS), names_to = "metric", values_to = "value")
#Check variables are stored as correct data type 
str(longrespdat)

#Correct variables
longrespdat$metric <- as.factor(longrespdat$metric)
longrespdat$group <- as.factor(longrespdat$group)
longrespdat$id <- as.factor(longrespdat$id)
